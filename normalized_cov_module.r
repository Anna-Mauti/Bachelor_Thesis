...
normalized_coverage <- function(data_sets, all_data, dir_out, file_name, color_palette) {
    ...
    for (c in 1:length(data_sets)) {
        ...
        plot <- ggplot(temp, aes(x = SVTYPE, y = fraction, fill = SVTYPE)) +
            ...
            labs(title = paste(data_sets[c], ": Normalized Coverage - Boxplot", sep = ""), x = "SV Type", y = "Support Per Read Depth") +
            theme_classic()
        print(plot)
        dev.off()
        ...
        plot <- ggplot(temp, aes(x = fraction, y = CHROM, color = SVTYPE)) +
            ...
            labs(title = paste(data_sets[c], ": Normalized Coverage with Chromosome Info - Scatter Plot", sep = ""), x = "Support Per Read Depth", y = "Chromosome", color = "SV Type") +
            theme_classic()
        print(plot)
        dev.off()
        ...
        plot <- ggplot(temp, aes(x = gt_DP, y = SUPPORT, color = SVTYPE)) +
            ...
            scale_color_manual(values = color_palette) +
            scale_size(range = c(5, 15)) +
            labs(title = paste(data_sets[c], ": Normalized Coverage - Scatter Plot", sep = ""), x = "Read Depth", y = "Support", color = "SV Type") +
            theme_classic() +
